class TweetsController < ApplicationController
	...
	def create
		...
		@user = current_user
		unless current_user.negarest && @newtweet.score < current_user.negarest_value
			@user.tweet_count += 1
		   ...
		   #ツイートできなかった時の処理
			...
			tweets_array = group_by_id.values.map{|a| a[0] }.sort_by! { |a| a[:updated_at] }.reverse!
			@tweets = Kaminari.paginate_array(tweets_array).page(params[:page]).per(10)
			flash[:warning] = "ネガティブツイート制限によりツイートを制限しました。[制限値:" + @user.negarest_value.to_s + " ツイートの感情指数:" + @newtweet.score.round.to_s + "]"
		end
end
def show
